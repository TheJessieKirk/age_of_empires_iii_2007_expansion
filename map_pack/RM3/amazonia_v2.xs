...
void main(void) {
    ...
    if (rmAllocateSubCivs(3) == true) {
        ...
        if (subCiv0 >= 0) rmSetSubCiv(0, "Jesuit");
        subCiv1 = rmGetCivID("Jesuit");
        rmEchoInfo("subCiv1 is Jesuit" + subCiv1);
        if (subCiv1 >= 0) rmSetSubCiv(1, "Tupi");
        subCiv2 = rmGetCivID("Tupi");
        rmEchoInfo("subCiv2 is Tupi" + subCiv2);
        if (subCiv2 >= 0) rmSetSubCiv(2, "Tupi");
    }

    // Picks the map size
    int playerTiles = 11000;
    if (cNumberNonGaiaPlayers > 4) playerTiles = 10000;
    if (cNumberNonGaiaPlayers > 6) playerTiles = 8000;
 ...
 */
    ...
    if (areaSizerNum > 6) areaSizer = 0.38;
...
*/
    ...
    if (tradeRouteNum > 3) tradeRouteNum = 2; //this makes the dual trade route option come up more often
    ...
    if (tradeRouteNum == 1) {
        ...
        if (placedTradeRoute == false) rmEchoError("Failed to place trade route");
        ...
        if (placedTradeRoute2 == false) rmEchoError("Failed to place trade route");
        ...
        if (placedTradeRoute3 == false) rmEchoError("Failed to place trade route");
        ...
        if (placedTradeRoute4 == false) rmEchoError("Failed to place trade route");
    ...
    for (i = 1; < cNumberPlayers) {
        ...
        if (rmGetNomadStart() == false) {
            if (ypIsAsian(i)) {
                ...
                rmPlaceObjectDefAtLoc(playerAsianMarketID, i, rmXMetersToFraction(xsVectorGetX(TCLoc)), rmZMetersToFraction(xsVectorGetZ(TCLoc)));
            } else rmPlaceObjectDefAtLoc(playerMarketID, i, rmXMetersToFraction(xsVectorGetX(TCLoc)), rmZMetersToFraction(xsVectorGetZ(TCLoc)));
        }
